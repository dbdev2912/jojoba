        ...
        if(validateData){
            ...
            if( brands.length > 0 ){
                ...
                if(logo){
                    ...
                    const oldPath = brand.logo;                                        
                    const logoPath = `public/${ oldPath }`
                ...
                const updateQuery = `
                    UPDATE THUONGHIEU SET 
                        ten_thuong_hieu = '${ brand_name }',
                        email = '${brand_email}',
                        hotline = '${brand_hotline}',
                        th_ghi_chu = '${ note }'
                    WHERE ma_thuong_hieu = '${ brand_id }'
                `;
                await MySQL_QUERY( updateQuery );
                res.redirect(`/admin/brands/edit/${ brand_id }?success=1`)
            }else{
                res.render('admin_404_not_found', {
                    layout: "admin",
                    title: `404 - Not found| ${ COMPANY }`,
